...
The quote above represents a memory during my time working in at influential and central hospital on the Hawaiian islands. Space has always been an issue and as such, the number of patients healthcare providers can provide is limited to space within the hospitals. Sometimes, patients may have to wait for a bed to clear or get squeezed into another patient room, obstructing the healing journey. Therefore, this decision by the hospital is subjective. There needs to be a more objective and quantitative rubric for admitting patients into the hospital, especially after surgery. 
...
revalue(data$deci, c('I' = 1, 
                  ...
                  'A' = 3)) -> data$deci
```
Interesting, observation [3,9] is still an "**A**'. Need to convert manually.
...
SVMs are used for two-group classifications. In laymen terms, it finds the best fit line of separation called the 'hyperplane."

Advantages of SVM:

* Useful for high dimensional spaces
* High speed and better performance if liminted observations (thosuands)
